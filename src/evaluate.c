...
const int PawnDoubled  = S( -8,-46);
const int PawnIsolated = S(-10, -8);
const int PawnSupport  = S( 20, 15);
const int PawnThreat   = S( 58, 55);
const int PushThreat   = S( 24, -3);
const int PawnOpen     = S(-14,-14);
const int BishopPair   = S( 31,104);
const int KingAtkPawn  = S( 48, 68);
const int OpenForward  = S( 26, 24);
const int SemiForward  = S( 12, 18);
const int NBBehindPawn = S( 11, 33);

// Passed pawn
const int PawnPassed[RANK_NB] = {
    S(  0,  0), S( -7, 13), S(-11, 24), S( -9, 62),
    S( 14,102), S( 28,180), S(135,239), S(  0,  0),
};

// Pawn phalanx
const int PawnPhalanx[RANK_NB] = {
    S(  0,  0), S( 11, -2), S( 18, 12), S( 32, 30),
    S( 65, 93), S( 99,196), S(164,251), S(  0,  0),
};

// KingLineDanger
const int KingLineDanger[28] = {
    S(  0,  0), S(  0,  0), S( 13,  6), S(  3, 33),
    S(-18, 42), S(-25, 37), S(-28, 34), S(-35, 41),
    S(-44, 43), S(-58, 44), S(-54, 37), S(-63, 39),
    S(-65, 38), S(-68, 35), S(-70, 34), S(-61, 27),
    S(-57, 25), S(-45, 15), S(-38, 10), S(-36,  3),
    S(-33, -1), S(-39, -6), S(-42, -6), S(-62,-21),
    S(-80,-30), S(-97,-44), S(-101,-41), S(-106,-43),
};

// Mobility [pt-2][mobility]
const int Mobility[4][28] = {
    // Knight (0-8)
    { S(-44,-97), S(-30,-46), S(-10, 13), S(  0, 39), S( 14, 40), S( 23, 49), S( 32, 49), S( 43, 46),
      S( 54, 30) },
    // Bishop (0-13)
    { S(-39,-132), S(-17,-80), S( -7, -9), S(  0, 22), S( 11, 34), S( 23, 49), S( 28, 60), S( 32, 65),
      S( 32, 74), S( 34, 75), S( 36, 75), S( 49, 64), S( 52, 65), S( 82, 40) },
    // Rook (0-14)
    { S(-78,-90), S(-13,-78), S(  1,-14), S(  4,  7), S(  2, 43), S(  6, 58), S(  5, 66), S( 11, 66),
      S( 13, 71), S( 19, 74), S( 26, 78), S( 23, 82), S( 21, 86), S( 31, 82), S( 78, 61) },
    // Queen (0-27)
    { S(-62,-48), S(-84,-48), S(-65,-89), S(-21,-96), S(  1,-90), S(  8,-62), S( 16,-16), S( 17, 15),
      S( 20, 40), S( 24, 52), S( 25, 61), S( 27, 69), S( 32, 70), S( 32, 79), S( 33, 88), S( 35, 90),
      S( 33,101), S( 32,107), S( 29,114), S( 32,115), S( 38,112), S( 40,108), S( 47, 98), S( 63, 77),
      S( 74, 60), S( 87, 58), S( 94, 95), S( 98,120) }
};
