...
INLINE int evalBishops(const EvalInfo *ei, const Position *pos, const int color) {
    ...
    for (int i = 0; i < pos->pieceCounts[bishops]; ++i) {
        ...
        // Mobility
        eval += BishopMobility[PopCount(BishopAttacks(sq, pos->pieceBBs[ALL]) & ei->mobilityArea[color])];
    }
...
INLINE int evalRooks(const EvalInfo *ei, const Position *pos, const int color) {
    ...
    for (int i = 0; i < pos->pieceCounts[rooks]; ++i) {
        ...
        // Mobility
        eval += RookMobility[PopCount(RookAttacks(sq, pos->pieceBBs[ALL]) & ei->mobilityArea[color])];
    }
...
INLINE int evalQueens(const EvalInfo *ei, const Position *pos, const int color) {
    ...
    for (int i = 0; i < pos->pieceCounts[queens]; ++i) {
        ...
        // Mobility
        eval += QueenMobility[PopCount((BishopAttacks(sq, pos->pieceBBs[ALL])
                                        | RookAttacks(sq, pos->pieceBBs[ALL])) & ei->mobilityArea[color])];
    }
...
int EvalPosition(const Position *pos) {
    ...
    // Mobility area
    blockedPawns[BLACK] = ei.pawnsBB[BLACK] & pos->pieceBBs[ALL] << 8;
    blockedPawns[WHITE] = ei.pawnsBB[WHITE] & pos->pieceBBs[ALL] >> 8;
