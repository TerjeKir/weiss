...
const int PawnDoubled    = S( -9,-31);
const int PawnIsolated   = S(-15,-15);
const int PawnSupport    = S( 13,  4);
const int BishopPair     = S( 24, 90);
const int KingLineDanger = S( -3, -2);

// Passed pawn [rank]
const int PawnPassed[8] = {
    S(  0,  0), S( -5, 11), S(-11, 17), S( -3, 44),
    S( 29, 68), S( 73,126), S(129,171), S(  0,  0),
};

// (Semi) open file for rook and queen [pt-4]
const int OpenFile[2]     = { S( 38, 12), S( -2,  4) };
const int SemiOpenFile[2] = { S( 16, 13), S(  2,  5) };

// Mobility [pt-2][mobility]
const int Mobility[4][28] = {
    // Knight (0-8)
    { S(-82,-52), S(-29,-50), S(  0,-28), S( 13, -1), S( 25, 16), S( 29, 40), S( 37, 46), S( 46, 48),
      S( 61, 30) },
    // Bishop (0-13)
    { S(-48,-63), S(-17,-57), S( -7,-37), S(  1,-18), S( 13, -2), S( 22, 24), S( 26, 40), S( 27, 48),
      S( 29, 55), S( 36, 56), S( 50, 51), S( 62, 44), S( 61, 80), S( 45, 59) },
    // Rook (0-14)
    { S(-59,-69), S(-23,-35), S( -7,-22), S( -8,-11), S( -4,  4), S( -1, 22), S(  1, 38), S(  7, 44),
      S( 13, 52), S( 19, 57), S( 27, 63), S( 34, 62), S( 45, 61), S( 59, 52), S( 66, 40) },
    // Queen (0-27)
    { S(-62,-48), S(-63,-35), S(-51,-44), S(-33,-44), S(-52,-40), S(-25,-38), S(-15,-32), S( -7,-23),
      S( -2, -1), S(  6,  1), S( 11, 18), S( 18, 20), S( 21, 29), S( 26, 38), S( 30, 43), S( 30, 53),
      S( 32, 57), S( 40, 51), S( 35, 66), S( 41, 66), S( 50, 83), S( 68, 75), S( 72, 79), S( 89, 74),
