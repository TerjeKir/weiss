...
const int KingAtkPawn  = S( 60, 78);
const int OpenForward  = S( 27, 11);
const int SemiForward  = S(  7, 15);
const int NBBehindPawn = S( 11, 37);

// Passed pawn [rank]
const int PawnPassed[8] = {
    S(  0,  0), S(-14, 23), S(-17, 27), S(-10, 60),
    S( 23, 90), S( 47,158), S(154,212), S(  0,  0),
};

// KingLineDanger
const int KingLineDanger[28] = {
    S(  0,  0), S(  0,  0), S( -1,  0), S(-12, 45),
    S(-30, 42), S(-35, 32), S(-35, 29), S(-37, 32),
    S(-41, 33), S(-52, 35), S(-46, 30), S(-58, 37),
    S(-58, 33), S(-61, 33), S(-62, 31), S(-54, 28),
    S(-46, 23), S(-33, 14), S(-27,  8), S(-30,  3),
    S(-36,  1), S(-43, -8), S(-53,-14), S(-69,-26),
    S(-90,-36), S(-106,-44), S(-107,-43), S(-112,-45),
};

// Mobility [pt-2][mobility]
const int Mobility[4][28] = {
    // Knight (0-8)
    { S(-38,-78), S(-22,-68), S( -3,-11), S(  7, 23), S( 16, 35), S( 19, 52), S( 27, 53), S( 38, 47),
      S( 54, 25) },
    // Bishop (0-13)
    { S(-49,-137), S(-17,-100), S( -3,-28), S(  5,  7), S( 13, 24), S( 21, 47), S( 25, 63), S( 24, 70),
      S( 22, 78), S( 27, 78), S( 28, 77), S( 52, 61), S( 61, 68), S( 75, 42) },
    // Rook (0-14)
    { S(-69,-79), S(-21,-91), S( -5,-38), S( -6,-15), S(  0, 20), S(  2, 40), S(  2, 60), S(  7, 62),
      S( 13, 67), S( 21, 71), S( 29, 76), S( 32, 77), S( 35, 80), S( 48, 70), S(103, 37) },
    // Queen (0-27)
    { S(-62,-48), S(-84,-43), S(-70,-71), S(-26,-74), S( -5,-73), S( 12,-64), S( 18,-32), S( 21, -1),
      S( 25, 19), S( 29, 39), S( 31, 54), S( 34, 63), S( 36, 68), S( 36, 75), S( 37, 80), S( 37, 85),
      S( 34, 90), S( 30, 95), S( 25, 99), S( 24, 99), S( 27, 96), S( 31, 86), S( 41, 76), S( 59, 59),
      S( 77, 57), S( 93, 61), S( 97,101), S(104,126) }
};
