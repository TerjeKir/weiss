...
const int PawnDoubled  = S(-11,-44);
const int PawnIsolated = S( -7,-16);
const int PawnSupport  = S( 18, 10);
const int PawnThreat   = S( 49, 70);
const int PushThreat   = S( 17,  3);
const int PawnOpen     = S(-14,-15);
const int BishopPair   = S( 23,116);
const int KingAtkPawn  = S( 48, 85);
const int OpenForward  = S( 26, 17);
const int SemiForward  = S(  9, 18);
const int NBBehindPawn = S(  9, 40);

// Passed pawn
const int PawnPassed[RANK_NB] = {
    S(  0,  0), S(-13, 25), S(-11, 33), S( -9, 72),
    S( 21,112), S( 38,193), S(131,275), S(  0,  0),
};

// Pawn phalanx
const int PawnPhalanx[RANK_NB] = {
    S(  0,  0), S(  8, -2), S( 17,  6), S( 23, 30),
    S( 55,117), S(158,263), S(179,385), S(  0,  0),
};

// KingLineDanger
const int KingLineDanger[28] = {
    S(  0,  0), S(  0,  0), S( -5, 15), S(-14, 51),
    S(-28, 50), S(-32, 37), S(-30, 32), S(-36, 40),
    S(-40, 39), S(-51, 42), S(-46, 35), S(-60, 44),
    S(-61, 42), S(-70, 42), S(-76, 42), S(-73, 41),
    S(-79, 39), S(-76, 30), S(-79, 25), S(-85, 20),
    S(-90, 16), S(-108, 10), S(-109,  1), S(-139,-12),
    S(-131,-31), S(-124,-49), S(-118,-34), S(-123,-36),
};

// Mobility [pt-2][mobility]
const int Mobility[4][28] = {
    // Knight (0-8)
    { S(-36,-143), S(-30,-31), S(-10, 30), S( -1, 63), S(  8, 73), S( 10, 91), S( 17, 91), S( 25, 86),
      S( 38, 61) },
    // Bishop (0-13)
    { S(-40,-98), S(-21,-62), S(-11, -5), S( -5, 31), S(  4, 47), S( 13, 73), S( 17, 91), S( 17, 97),
      S( 15,109), S( 20,109), S( 23,107), S( 46, 90), S( 45, 98), S( 95, 52) },
    // Rook (0-14)
    { S(-111,-167), S(-24,-25), S( -7, 35), S( -6, 41), S( -6, 75), S( -1, 91), S( -3,110), S(  3,111),
      S(  8,118), S( 15,123), S( 23,129), S( 24,133), S( 25,137), S( 37,127), S( 74,101) },
    // Queen (0-27)
    { S(-64,-48), S(-104,-57), S(-98,-115), S(-35,-141), S(-12,-63), S(-12, 35), S( -9, 80), S( -5,109),
      S( -2,131), S(  2,148), S(  4,162), S(  6,175), S(  9,178), S( 10,184), S( 11,189), S( 12,194),
      S( 11,199), S(  9,202), S(  5,210), S(  3,215), S( 10,210), S(  2,218), S( 16,211), S( 11,211),
      S( 64,172), S( 71,156), S(116,143), S(104,148) }
};
