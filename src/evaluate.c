...
const int PawnDoubled  = S(-11,-42);
const int PawnIsolated = S( -8,-14);
const int PawnSupport  = S( 19,  9);
const int PawnThreat   = S( 49, 67);
const int PushThreat   = S( 18,  2);
const int PawnOpen     = S(-14,-13);
const int BishopPair   = S( 24,112);
const int KingAtkPawn  = S( 54, 79);
const int OpenForward  = S( 24, 21);
const int SemiForward  = S(  8, 20);
const int NBBehindPawn = S(  6, 42);

// Passed pawn
const int PawnPassed[RANK_NB] = {
    S(  0,  0), S(-12, 23), S(-10, 30), S( -7, 68),
    S( 23,105), S( 43,179), S(142,252), S(  0,  0),
};

// Pawn phalanx
const int PawnPhalanx[RANK_NB] = {
    S(  0,  0), S(  7, -2), S( 18,  5), S( 23, 28),
    S( 58,107), S(125,226), S(166,266), S(  0,  0),
};

// KingLineDanger
const int KingLineDanger[28] = {
    S(  0,  0), S(  0,  0), S( -8, 13), S(-18, 51),
    S(-33, 50), S(-37, 38), S(-35, 33), S(-41, 42),
    S(-45, 41), S(-56, 44), S(-51, 38), S(-64, 46),
    S(-65, 44), S(-73, 44), S(-77, 44), S(-72, 41),
    S(-74, 39), S(-70, 29), S(-69, 24), S(-70, 18),
    S(-69, 12), S(-79,  5), S(-79, -4), S(-97,-19),
    S(-106,-33), S(-117,-46), S(-118,-34), S(-123,-36),
};

// Mobility [pt-2][mobility]
const int Mobility[4][28] = {
    // Knight (0-8)
    { S(-39,-112), S(-25,-45), S( -5, 14), S(  4, 45), S( 13, 55), S( 16, 72), S( 23, 71), S( 31, 66),
      S( 44, 41) },
    // Bishop (0-13)
    { S(-36,-112), S(-18,-70), S( -8,-15), S( -2, 20), S(  8, 36), S( 17, 60), S( 21, 77), S( 21, 84),
      S( 20, 94), S( 25, 94), S( 29, 92), S( 52, 76), S( 54, 80), S( 91, 42) },
    // Rook (0-14)
    { S(-85,-100), S(-20,-49), S( -5, 18), S( -4, 26), S( -3, 60), S(  1, 76), S( -1, 95), S(  6, 96),
      S( 11,102), S( 18,106), S( 26,111), S( 29,114), S( 30,117), S( 42,107), S( 76, 84) },
    // Queen (0-27)
    { S(-62,-48), S(-85,-50), S(-74,-94), S(-28,-107), S(  7,-96), S( 15,-52), S( 18, -2), S( 21, 32),
      S( 24, 55), S( 28, 74), S( 30, 88), S( 32,101), S( 35,105), S( 35,113), S( 36,119), S( 37,124),
      S( 36,132), S( 33,136), S( 30,142), S( 29,146), S( 37,140), S( 38,138), S( 55,124), S( 69,104),
      S( 90, 83), S( 95, 73), S( 96, 99), S( 96,119) }
};
