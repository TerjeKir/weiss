...
float LMRNoisyBase = 0.48;
float LMRNoisyDiv = 3.43;
float LMRQuietBase = 1.92;
float LMRQuietDiv = 2.46;
...
float NodeRatioBase = 55;
float NodeRatioMult = 301;
...
int HistMaCDiv = 1024;
int HistCCDiv = 1220;
int HistNPCHDiv = 1024;
int HistBonusMax = 2410;
int HistBonusBase = 310;
int HistBonusDepth = 268;
int HistMalusMax = 834;
int HistMalusBase = 148;
int HistMalusDepth = 531;
int HistCBonusDepthDiv = 4;
int HistCBonusMax = 254;
int HistCBonusMin = 212;
int HistGetPC  = 6554;
int HistGetMiC = 6800;
int HistGetMaC = 3700;
int HistGetNPC = 7000;
...
static int AlphaBeta(Thread *thread, Stack *ss, int alpha, int beta, Depth depth, bool cutnode) {
    ...
    if (   depth >= ProbCutDepth
        ...
        while ((move = NextMove(&mp))) {
            ...
            if (score >= probCutBeta)
                return isWin(score) ? score : MAX(beta, score - ProbCutReturn);
        }
    }
